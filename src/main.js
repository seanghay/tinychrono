...
function formatTimeRange({ type: _type, hours, day: _day }, options = {}) {
  ...
  if (!type)
    throw new Error(
      "type is invalid. please use either FULL_DAY, CLOSED, or RANGE"
    );
  ...
  if (type === "RANGE") {
    ...
    if (!hours.every((hourRange) => hourRange.length >= 2)) {
      throw new Error(
        "Some hour value is invalid. \n" +
          "Each hour must have at least two value. JSON: \n" +
          JSON.stringify(hours, null, 2)
      );
    }

    const rangeString = hours
      .map(parseIntRange)
      .map(
        ([from, to]) =>
          `${decodeMilitaryHour(from)} - ${decodeMilitaryHour(to)}`
      )
      .join(", ");
...
function formatDays(items = [], options = {}) {
  ...
  if (_autofill) {
    if (typeof _autofill === "function") {
      items = fillMissingDays(items, _autofill);
    } else {
